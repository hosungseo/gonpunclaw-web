...
import { validateDashboardPayload } from '@/lib/demo-validation';
...
const dashboardData = validateDashboardPayload(demoData, crossData);

export default function DashboardPage() {
  if (!dashboardData.ok) {
    return <DashboardDataError reason={dashboardData.reason} />;
  }

  const { demo, cross } = dashboardData;
  const { apt, air, gazette, fetchedAt } = demo;
  const sejong = cross.sejong;
  const lawCross = cross.law_cross;
  const catalog = cross.catalog;
        ...
        <section className="mb-14">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {apt.length > 0 ? (
              ...
              <EmptyStateCard
                title="아파트 실거래가 데이터 없음"
                description="demo.json의 apt 배열이 비어 있습니다. 수집 스크립트나 API 응답을 확인해주세요."
              />
            )}
          </div>
        </section>
        ...
        <section className="mb-14">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {air.length > 0 ? (
              ...
              <EmptyStateCard
                title="미세먼지 데이터 없음"
                description="측정소 데이터가 아직 수집되지 않았습니다."
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
                <StatCard
                  ...
                  icon="🏠"
                  label="아파트 매매"
                  value={`${sejong.apt?.avg_price?.toLocaleString() ?? '—'}만원`}
                  sub={`세종시 2025.2월 평균 · ${sejong.apt?.count ?? 0}건`}
                />
                <StatCard
                  color="emerald"
                  icon="🌫️"
                  label="미세먼지"
                  value={`${sejong.air?.[0]?.PM10 ?? '—'}㎍/㎥`}
                  sub="세종시 PM10 실시간"
                />
                <StatCard
                  color="purple"
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                ...
                {sejong.apt.data.length > 0 ? (
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {sejong.apt.data.slice(0, 6).map((item, i) => (
                        <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                  ...
                  <InlineEmptyState title="세종시 아파트 상세 데이터 없음" description="교차분석 카드 값은 있지만 상세 거래 목록은 비어 있습니다." />
                )}
              </div>
            </>
          ) : (
            <EmptyStateCard
              title="세종시 교차분석 데이터 없음"
              description="cross.sejong 값이 없어도 섹션은 유지합니다. 데이터 파이프라인이 준비되면 이 카드가 실제 차트/표로 대체됩니다."
            />
          )}
        </section>
      </FadeIn>
          ...
          {lawCross ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <CrossCard color="indigo" title="📚 법제처 법령">
                {lawCross.laws.length > 0 ? lawCross.laws.map((l, i) => (
                ...
                )) : <InlineEmptyState title="법제처 법령 없음" description="관련 법령 응답이 비어 있습니다." />}
              </CrossCard>
              <CrossCard color="gray" title="📜 관보 고시">
                {lawCross.gazette.length > 0 ? lawCross.gazette.map((g, i) => (
                  <div key={i} className="text-xs border-b border-gray-100 pb-2 last:border-0">
                    <div className="font-medium text-gray-900 mb-0.5">{g.제목}</div>
                    <div className="text-gray-400">{g.기관} · {g.발행일}</div>
                  </div>
                )) : <InlineEmptyState title="관보 고시 없음" description="해당 기간에 연결된 관보 데이터가 아직 없습니다." />}
              </CrossCard>
              <CrossCard color="emerald" title="🏛️ 국회 법안">
                {lawCross.bills.length > 0 ? lawCross.bills.map((b, i) => (
                  <div key={i} className="text-xs border-b border-gray-100 pb-2 last:border-0">
                    <div className="font-medium text-gray-900 mb-0.5">{b.법안명}</div>
                    <div className="text-gray-400">{b.발의자} · {b.제안일}</div>
                  </div>
                )) : <InlineEmptyState title="국회 법안 없음" description="연결된 입법 데이터가 비어 있습니다." />}
              </CrossCard>
            </div>
          ) : (
            <EmptyStateCard
              title="법령·관보·국회 교차분석 데이터 없음"
              description="cross.law_cross 값이 누락되어도 섹션을 유지해 빈 상태를 명확히 보여줍니다."
            />
          )}
        </section>
      </FadeIn>
      ...
      <FadeIn delay={0.05}>
        <section className="mb-14">
          <SectionHeader
            ...
            sub={`공공데이터포털 목록조회 API — 전체 ${catalog?.total_apis?.toLocaleString() ?? 0}개 오픈API`}
          />
          {catalog ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                <h3 className="text-sm font-bold text-gray-800 mb-4">기관별 API 등록 수 (상위 10)</h3>
                {catalog.top_orgs.length > 0 ? catalog.top_orgs.map((o, i) => (
                ...
                )) : <InlineEmptyState title="기관별 통계 없음" description="top_orgs 배열이 비어 있습니다." />}
              </div>
              <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                <h3 className="text-sm font-bold text-gray-800 mb-4">최근 등록 API 샘플</h3>
                {catalog.sample.length > 0 ? catalog.sample.map((s, i) => (
                ...
                )) : <InlineEmptyState title="API 샘플 없음" description="최근 등록 API 샘플 데이터가 아직 없습니다." />}
                <div className="mt-4 text-center text-xs text-gray-400">
                  전체 {catalog.total_apis?.toLocaleString()}개 오픈API 중 샘플
                </div>
              </div>
            </div>
          ) : (
            <EmptyStateCard
              title="API 카탈로그 데이터 없음"
              description="cross.catalog 값이 누락되어도 섹션은 남겨 두고 수집 상태를 바로 확인할 수 있게 했습니다."
            />
          )}
        </section>
      </FadeIn>
...
function DashboardDataError({ reason }: { reason: string }) {
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
      <div className="rounded-3xl border border-red-100 bg-red-50/70 p-8 shadow-sm">
        <p className="text-xs font-mono text-red-500 tracking-widest uppercase mb-3">Dashboard fallback</p>
        <h1 className="text-2xl font-bold text-gray-900 mb-3">대시보드 데이터를 불러올 수 없습니다</h1>
        <p className="text-sm text-gray-600 leading-6">
          JSON 구조 검증에 실패했습니다. 잘못된 데이터가 조용히 렌더링되는 대신, 지금은 전체 fallback UI를 보여줍니다.
        </p>
        <div className="mt-5 rounded-2xl bg-white border border-red-100 px-4 py-3 text-sm text-gray-700">
          {reason}
        </div>
        <p className="mt-4 text-xs text-gray-400">확인 대상: src/data/demo.json, src/data/demo-cross.json</p>
      </div>
    </div>
  );
}
...
function EmptyStateCard({ title, description, className = '' }: { title: string; description: string; className?: string }) {
  return (
    <div className={`rounded-2xl border border-dashed border-gray-200 bg-white/80 p-6 shadow-sm ${className}`}>
